...
describe("KYCRegistry", function () {
  describe("Root Management", function () {
    it("should allow the KYC admin to update the Merkle root", async function () {
      const { kycRegistry, kycAdmin } = await loadFixture(
      ...
      await expect(kycRegistry.connect(kycAdmin).updateRoot(newRoot))
        ...
        .withArgs(
          await kycRegistry.merkleRoot(),
          ethers.utils.hexlify(newRoot)
        );

      expect(await kycRegistry.merkleRoot()).to.equal(
        ethers.utils.hexlify(newRoot)
      );
    });

    it("should prevent a non-admin from updating the Merkle root", async function () {
      const { kycRegistry, otherUser } = await loadFixture(
        deployContractsFixture
      );
      const newRoot = ethers.utils.randomBytes(32);
      ...
      await expect(
        kycRegistry.connect(otherUser).updateRoot(newRoot)
      ).to.be.revertedWith(
        `AccessControl: account ${otherUser.address.toLowerCase()} is missing role ${KYC_ADMIN_ROLE}`
      );
    });
  });
  ...
  describe("Proof Verification", function () {
    it("should allow a valid user to prove their address and get verified", async function () {
      const { kycRegistry, user1, kycTree } = await loadFixture(
        deployContractsFixture
      );
    ...
    it("should not allow a user to get verified with an invalid proof", async function () {
      ...
      await expect(
        kycRegistry.connect(user1).proveAndUpdate(invalidProof)
      ).to.be.revertedWith("KYCR: Invalid proof");
    });

    it("should not allow an unlisted user to get verified", async function () {
      const { kycRegistry, otherUser, kycTree } = await loadFixture(
        deployContractsFixture
      );
      ...
      const proof = kycTree.getHexProof(
        keccak256((await ethers.getSigners())[5].address)
      ); // user1's leaf

      await expect(
        kycRegistry.connect(otherUser).proveAndUpdate(proof)
      ).to.be.revertedWith("KYCR: Invalid proof");
    });
    ...
    it("should not emit AddressVerified if user is already verified", async function () {
      ...
      await expect(
        kycRegistry.connect(user1).proveAndUpdate(proof)

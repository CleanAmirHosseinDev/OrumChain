...
describe("OracleAggregator", function () {
  ...
  it("should allow a valid oracle to update the price", async function () {
    const { oracleAggregator, oracle } = await loadFixture(
  ...
  it("should prevent a non-oracle from updating the price", async function () {
    const { oracleAggregator, otherUser } = await loadFixture(
      deployContractsFixture
    );
    ...
    await expect(
      oracleAggregator.updatePrice(newPrice, timestamp, signature)
  ...
  it("should prevent updates with a stale timestamp", async function () {
    ...
    const signature1 = await getOracleSignature(
      oracle,
      oracleAggregator.address,
      price1,
      timestamp1
    ...
    const signature2 = await getOracleSignature(
      oracle,
      oracleAggregator.address,
      price2,
      timestamp1
    ...
    await expect(
      oracleAggregator.updatePrice(price2, timestamp1, signature2)
    ...
    const signature3 = await getOracleSignature(
      oracle,
      oracleAggregator.address,
      price3,
      timestamp3
    ...
    await expect(
      oracleAggregator.updatePrice(price3, timestamp3, signature3)
    ).to.be.revertedWith("OA: Stale timestamp");
  });
});
